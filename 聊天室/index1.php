	...
	*/
	...
	function openFolderWebPath(){	//获取请求php文件在http的位置
		...
		foreach ($urls as $i){
			...
			if ($i == $php_self_name or $i == "openfile" or $i == "t"){//如果遇见这个php脚本名称标记一下
				$meet_php = 1;//True;
			}
		}
		return $last_path;
	...
	function Page404(){
		...
		echo '<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><title>基于php的网络文件存储系统</title></head>';
		...
		if (is_dir($path)){
			...
			foreach ($temp_list as $f_name){	//便利
				if ($f_name != ".." && $f_name != "."){	//排除根目录
					...
					//文件名称
					if (is_dir($path . "/" . $f_name)){	
						echo '<td><a href="'.$php_web_path."/file".$web_path.$f_name.'/">'.$f_name.'</a></td>';
					} else {
						//echo "<td>".$f_name."</td>";
						echo '<td><a href="'.$php_web_path."/file".$web_path.$f_name.'">'.$f_name.'</a></td>';
					}
			...
			//输出HTML
			echo '<h4>文件预览</h4>';
			echo '<iframe name="ifd" onload="this.height=ifd.document.body.scrollHeight+100;if(this.height>1000){this.height=1000}" width="100%" frameborder="0" src="/index.php/openfile'.$web_path.'"></iframe>';
		
		} else {
			echo '<h2>当前文件或文件夹不存在</h2>';
		}
	...
	switch ($getReqType)
	{
	case "": case "file": folderViews(); break; // / /folder
