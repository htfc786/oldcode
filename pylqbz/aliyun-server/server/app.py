...
def bmi():
    with open('request_cs.txt','r') as f:
        cs = int(f.read())
    cs += 1
    ...
    return render_template('homepage.html',bmi=context['bmi'],tips=context['tips'],cs=cs)


filegx.init(app=app,rootdir='/root/data/flask-server-data/file')
pythonxkt.init(app=app,rootdir='/root/data/pythonxkt')
