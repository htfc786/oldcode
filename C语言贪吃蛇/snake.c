...
#define INIT_SIZE 10  // 蛇的初始长度
#define INIT_TICK 50  // 初始tick   游戏一个画面持续时间（毫秒）
...
void g_gotoxy(int x,int y){  // 游戏内的移动函数 
	...
	//c_gotoxy(x_c,y_c);
	//printf("     %f %f",x_c,y_c);
	if((y_c < 0 || x_c < WIDTH || y_c < 0 || y_c < HIGHT)){
		c_gotoxy(x_c,y_c);
	}
...
void draw_wall(){  // 绘制游戏界面--边框 
	for (int i=2;i < HIGHT+2;i++){  // 绘制左边框 
		c_gotoxy(0,i);
		printf(I_WALL_Y);
	} 
	for (int i=2;i < HIGHT+2;i++){  // 绘制右边框 
		c_gotoxy(WIDTH+2,i);
		printf(I_WALL_Y);
		printf("%d",i-2);
	} 
...
void Astar_Pathfinding(){
	int snake_up = jdz(apple.X - body[0].X) + jdz(apple.Y - body[0].Y-1);
	int snake_down = jdz(body[0].X - apple.X) + jdz(body[0].Y+1 - apple.Y);
	int snake_right = jdz(body[0].X-1 - apple.X) + jdz(body[0].Y - apple.Y);
	int snake_left = jdz(body[0].X+1 - apple.X) + jdz(body[0].Y - apple.Y);
...
int main(){
	...
	while(1){		
		if(state == 0){
			...
			printf("(按esc按键退出)");
			/* 
			g_gotoxy(CHOICE_P_X-10,CHOICE_P_Y+9);
			printf("(按esc按键退出)");
			g_gotoxy(CHOICE_P_X-26,CHOICE_P_Y+9);
			printf("游戏玩法介绍",tick); 
			g_gotoxy(CHOICE_P_X+10,CHOICE_P_Y+9);
			printf("速度(数值越小越快) [%d   ]",tick);//"        "*/
			while(1){
				get_key();
				if(difficulty == 0){
				...
				if (key_type == 1){         // 向上 
					difficulty -= 1;
					if (difficulty == 0-1) difficulty = 2;
					key_type = 0;
				} else if (key_type == 2){  // 向下
					difficulty += 1;
					if (difficulty == 2+1) difficulty = 0;
					key_type = 0;
		...
		} else if(is_death && state == 2){  // 判断是否gameover 
