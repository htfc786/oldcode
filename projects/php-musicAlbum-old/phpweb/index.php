...
<body>
    <div id="big-border">
        ...
        <h3 id="little-page-title"><a href='/users/logout.php'>登出</a></h3>
        <div id="msg"></div>
        <hr>
        <div>
            <div>
            ...
            if (isset($_SESSION['islogin'])) {
                ...
                $sql = " select id,albumName,albumCreateDate,isDel from album where albumMreatorId = $userid";
                $rsaaa = mysqli_query($db,$sql);  //执行sql！！！
                //$num = mysqli_num_rows($rs);  //获取有多少个
                //$row = mysqli_fetch_array($rs,1);
                while ($row=mysqli_fetch_assoc($rsaaa)){
                    //echo $num."</br>";
                    $albumId = $row["id"];
                    ...
                    echo <<<END
                    <div class="margin5" style="position: relative;">
                        <a onclick="window.open('/showalbum.php/$albumId','_blank');">
                            <fieldset style="width:300px;height: 120px;">
                                <div style="font-size:20px;">$albumName</div>   
                                <div style="font-size:10px;color:#999;">创建时间：$albumCreateDate</div>
                                <div style="font-size:10px;color:#999;">$albumPhotoNum 张照片</div>
                                <a onclick="delAlbum($albumId);">删除</a>
                                <div onclick="window.location.href='/showphoto.php/$albumId';" style="position: absolute;left: 10px;width: 140px;height: 40px;line-height: 40px;top: 90px;border: 1px solid #444;text-align: center;color: #444;border-radius: 6px 6px 6px 6px;font-size: 18px;">提取图片</div>
                                <div onclick="window.location.href='/makealbum.php/$albumId';" style="background-color: #444;border-radius: 6px;width: 140px;height: 42px;position: absolute;color: #fff;text-align: center;line-height: 42px;font-size: 20px;top: 90px;left: 175px;">编辑</div>
                            </fieldset>
                        </a>
                    </div>
                    </br>
