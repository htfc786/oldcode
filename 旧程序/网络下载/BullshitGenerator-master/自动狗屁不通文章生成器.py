...
mrmy = data["famous"] # a 代表前面垫话，b代表后面垫话
qmdh = data["before"] # 在名人名言前面弄点废话
hmdh = data['after']  # 在名人名言后面弄点废话
fh = data['bosh'] # 代表文章主要废话来源
...
def havesomemrmy():
    global nextmrmy
    xx = next(nextmrmy)
    xx = xx.replace(  "a",random.choice(qmdh) )
    xx = xx.replace(  "b",random.choice(hmdh) )
    return xx
